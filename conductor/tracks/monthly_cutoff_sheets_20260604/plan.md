...
- [ ] Task: Refactor DB Summary Queries
    - [ ] Write failing unit tests in `tests/test_db.py` for fetching transactions and summaries filtered by a specific calendar month.
    - [ ] Implement `db.get_transactions_by_month(db_path, year, month)` and update `db.get_summaries(db_path, period, year, month)` in `db.py`.
