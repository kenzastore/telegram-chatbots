...
- [ ] Task: Create tests for Natural Language Date Parser
    - [ ] Write failing unit tests in `tests/quick.test.ts` for all relative, numeric, and textual date format variations, verifying they are correctly parsed into `YYYY-MM-DD` relative to Asia/Jakarta (GMT+7) timezone.
    - [ ] Write failing unit tests in `tests/quick.test.ts` for description cleanup, verifying leading and trailing prepositions are stripped correctly.
- [ ] Task: Align Parser Implementation
    - [ ] Refactor `quick.ts` to implement full date regex parsing and preposition cleanup, passing the unit tests.
    - [ ] Run `npm test` and verify that all test cases pass.
    - [ ] Ensure typescript type checks succeed.
