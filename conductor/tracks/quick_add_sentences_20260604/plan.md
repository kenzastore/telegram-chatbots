...
## Phase 3: Extended Date Parsing Support [checkpoint: 9607868]

- [x] Task: Write failing unit tests for extended date formats (92c8c6b)
    - [x] Add tests in `tests/test_bot.py` covering absolute date formats (`YYYY-MM-DD`, `DD-MM-YYYY`, `DD-MM`) and textual formats (`DD Month YYYY`, `DD Month`, `Month DD YYYY`, `Month DD`) in English and Indonesian.
- [x] Task: Implement extended date parsing in `bot.py` (80b9687)
    - [x] Update `parse_transaction_sentence` to identify, parse, and remove various date formats from the sentence description.
- [x] Task: Conductor - User Manual Verification 'Phase 3: Extended Date Parsing Support' (Protocol in workflow.md)
